...
export default {
  ...
  data() {
    return {
      form: {
        ...
        play_id: this.event.play_id || '',
        date: this.event.date.slice(0, 16), // Ajuste para datetime-local
        start_time: this.event.start_time || '',
        end_time: this.event.end_time || '',
        description: this.event.description || '',
        url_info: this.event.url_info || '',
        url_tickets: this.event.url_tickets || '',
  ...
  methods: {
    submitForm() {
      this.$inertia.put(`/eventos/${this.event.id}`, this.form, {
        onSuccess: () => {
